...
int ZombiePlugin::OnTakeDamage( const CTakeDamageInfo &info )
{
	...
	if ( pVictim && pEdict && !pEdict->IsFree() )
	{
		...
		iHealth -= info.m_flDamage;
		//g_Players[AIndex].iVictimList[nIndex] = g_Players[AIndex].iVictimList[nIndex] + (g_Players[nIndex].iHealth - iHealth);
		...
		if ( pAttacker )
		{
			...
			if ( attackerZombie && !isZombie )
			{
				...
				//if ( zombie_show_health.GetInt() > 0 )
				//{
				//	HintTextMsg( pAEdict, "You have %dhp left.", g_Players[AIndex].iHealth );
					//Hud_Print( pAEdict, "\x03[ZOMBIE]\x01 You have %dhp left.", g_Players[AIndex].iHealth );
				//}
			...
			else if ( isZombie )
			{
				...
				//int iHealth;
				//UTIL_GetProperty( g_Offsets.m_iHealth, pEnt, &iHealth );
				//iHealth = iHealth - info.m_flDamage;
				g_Players[AIndex].iVictimList[nIndex] = g_Players[AIndex].iVictimList[nIndex] + info.m_flDamage;
				//g_Players[nIndex].iHealth = iHealth;
				...
				//g_Players[x].iClass > -1 && g_Players[x].iClass <= g_iZombieClasses
		...
		else if ( FStrEq( sCmd, "menuselect" ) )
		{
			if ( g_Players[iPlayer].iClassMenu != -1 )
			{
				...
				if ( iCommand > 0 || ( iCmd != 0 && iCmd != 10 && ( iVal <= g_iZombieClasses ) ) )
				{
					...
					{
						if ( iVal == g_Players[iPlayer].iClass )
						{
							g_Players[iPlayer].iChangeToClass = -1;
						}
						else
						{
							g_Players[iPlayer].iChangeToClass = iVal;
						}

						IPlayerInfo *iInfo = m_PlayerInfoManager->GetPlayerInfo( pEntity );
						if ( iInfo )
						{
							kvPlayerClasses->SetInt( iInfo->GetNetworkIDString(), iVal );
						}
						ZombieClasses_t *c;
...
void ZombiePlugin::SetProtection( int iPlayer, bool bProtect, bool bReport, bool bRoundEnd )
{
	...
	if ( IsValidPlayer( iPlayer, &cEnt ) && IsValidPlayer( g_Players[iPlayer].pPlayer ) && IsAlive( cEnt ) && !(g_Players[iPlayer].isZombie && bProtect) )
	{
		...
		if ( bProtect )
		{
			//g_Players[iPlayer].tRenderMode = GetRenderMode( g_Players[iPlayer].pPlayer );
		...
		{
			tRenderMode = kRenderNormal; //g_Players[iPlayer].tRenderMode;
			cColour.a = 255;
			cColour.b = 255;
			cColour.g = 255;
			cColour.r = 255;
		...
		/*
		if ( !bProtect && !bRoundEnd ) 
		{
			META_LOG( g_PLAPI, "De-Protecting Player %d!", iPlayer );
		}
		*/
	...
	if ( IsValidPlayer( pEntity, &iPlayer ) )
	{
		...
		for ( x = 0; x <= MAX_PLAYERS; x++ )
		{
			if ( g_Players[iPlayer].iVictimList[x] > 0 )
			{
				...
				if ( sMenu.size() + Q_strlen( sTmp ) > 255 )
				{
					UTIL_DrawMenu( iPlayer, -1, zombie_damagelist_time.GetInt(), sMenu.c_str(), false );
					sMenu.assign( sTmp );
				}
				else
				{
		...
		if ( !bKiller && (iKiller > -1) )
		{
			...
			UTIL_GetProperty( g_Offsets.m_iHealth, pKiller, &iHealth );
			//damage_infector
			Q_snprintf( sTmp, sizeof(sTmp), GetLang("damage_infector"), g_Players[iKiller].sUserName.c_str(), iHealth );
			Q_snprintf( sTmp, sizeof(sTmp), " \n%s", sTmp ); //Infector: %s has %d left.
			if ( sMenu.size() + Q_strlen( sTmp ) > 255 )
			{
				UTIL_DrawMenu( iPlayer, -1, zombie_damagelist_time.GetInt(), sMenu.c_str(), false );
				sMenu.assign( sTmp );
...
void ZombiePlugin::ShowClassMenu( edict_t * pEntity )
{
	...
	#define ADDTOSTRING( ) \
		if ( sMenu.size() + Q_strlen( sTmp ) > 255 ) \
		{ \
			UTIL_DrawMenu( iPlayer, -1, 20, sMenu.c_str(), false ); \
			sMenu.assign( sTmp ); \
		} \
		else \
		{ \

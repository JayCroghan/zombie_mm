...
ConVar zombie_show_health( "zombie_show_health", "1.0", FCVAR_PLUGIN, "When 1 sends a custom message to a zombie every time they zombify someone displaying their current health, when 2 it uses HintText.", true, 0.0, true, 2.0 );
...
ConVar zombie_classes_random( "zombie_classes_random", "0.0", FCVAR_PLUGIN, "When 1, random class chosen each time a zombie dies. When 2, random zombie chosen each time a player connects. When 3 uses same behaviour as 1 but until player choses class themselves.", true, 0.0, true, 3.0 );
...
ConVar zombie_grenade_damage_multiplier( "zombie_grenade_damage_multiplier", "2.0", FCVAR_PLUGIN, "When enabled grenades do damage times this number. 1 disables", true, 0.0, true, 100.0 );
ConVar zombie_grenade_knockback_multiplier( "zombie_grenade_knockback_multiplier", "2.0", FCVAR_PLUGIN, "When enabled grenades have knockback times this number. 1 disables.", true, 0.0, true, 100.0 );
ConVar zombie_health_bonus( "zombie_health_bonus", "50", FCVAR_PLUGIN, "How much health to give zombies as a bonus for zombification. 0 disables.", true, 0.0, true, 5000.0 );
...
CON_COMMAND( zombie_listclasses, "List all currently loaded zombie classes." )
{
	...
	for ( x = 0; x <= g_iZombieClasses; x++ )
	{
		...
		META_CONPRINTF( "Headhosts Only: %s\n", (g_ZombieClasses[x].bHeadShotsOnly ? "Yes" : "No"));

		META_CONPRINTF( "Regen Health: %d\n", g_ZombieClasses[x].iRegenHealth );

		META_CONPRINTF( "Regen Seconds: %f\n", g_ZombieClasses[x].fRegenTimer );

		META_CONPRINTF( "Grenade Multiplier: %f\n", g_ZombieClasses[x].fGrenadeMultiplier );

		META_CONPRINTF( "Grenade Knockback: %f\n", g_ZombieClasses[x].fGrenadeKnockback );

		META_CONPRINTF( "Health Bonus: %d\n\n", g_ZombieClasses[x].iHealthBonus );
	}

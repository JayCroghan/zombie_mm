...
public:
    ...
    void Reset()
	{
        ...
        m_lastTimerTime = g_SMAPI->GetCGlobals()->realtime;
        m_lastTimerId = 0;
        m_inUse = false;
    }
    ...
    void CheckTimers(bool simulating)
	{
		...
		{
			g_fUniversalTime += g_SMAPI->GetCGlobals()->curtime - m_fLastTickedTime;
		}
		else 
		{
			g_fUniversalTime += g_SMAPI->GetCGlobals()->interval_per_tick;
		}

		m_fLastTickedTime = g_SMAPI->GetCGlobals()->curtime;
        ...
        //CheckUse();
        //m_inUse = true;
        //float curtime = g_SMAPI->GetCGlobals()->realtime;
